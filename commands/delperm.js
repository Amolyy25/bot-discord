...
const { removePermission } = require('./utils/permHelper');

module.exports = {
    data: new SlashCommandBuilder()
        .setName('delperm')
        .setDescription('Retire une permission à un utilisateur ou un rôle pour une commande')
        ...
        .addRoleOption(option =>
            option.setName('role')
                .setDescription('Le rôle à qui retirer la permission')
                .setRequired(false)),
    ...
    async executeMessage(message, args) {
        ...
        if (args.length < 2) {
            return message.reply('❌ Usage: `-delperm <commande> <@user|@role>`');
        }
        ...
        if (user) {
            removePermission(commandName, 'user', user.id);
            message.reply(`🗑️ Permission **${commandName}** retirée à ${user}.`);
        }
        
        if (role) {
            removePermission(commandName, 'role', role.id);
            message.reply(`🗑️ Permission **${commandName}** retirée au rôle ${role}.`);
        }
    }
};