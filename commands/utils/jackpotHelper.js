...
const CONFIG = {
    ...
    ROLES: [
        { id: '1471433193608712192', name: 'ಭ (Commun)', chance: 60 },
        { id: '1471431589081780224', name: '₪ (Rare)', chance: 25 },
        { id: '1471431515283132552', name: 'FIRST CLASSE (Très Rare)', chance: 10 },
        { id: '1471431323645378766', name: 'BUSINESS CLASS (Ultra Rare)', chance: 5 }
    ],
    REACTIONS: ['🚀', '💎', '🔥', '🎰', '⚡', '✈️', '🌟', '👑', '💰', '🚁']
};
...
async function launchJackpot(client, overrideChannelId) {
    try {
        ...
        if (!channel) return console.error(`[Jackpot] Salon ${channelId} introuvable`);
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`Un siège vient de se libérer dans l'avion du QG ! ✈️\nPremier à cliquer sur ${randomReaction} gagne son rang !\n\nBonne chance à tous !`)
        ...
        const message = await channel.send({ 
            content: '🚨 **JACKPOT CHRONO DÉTECTÉ !** Vite ! Soyez le premier à cliquer sur la réaction ci-dessous pour monter à bord ! ✈️',
            embeds: [embed] 
        });
        ...
        const collector = message.createReactionCollector({ filter, max: 1, time: 3600000 }); // 1h max pour cliquer
        ...
        collector.on('collect', async (reaction, user) => {
            ...
            try {
                ...
                await channel.send(`🏆 **FÉLICITATIONS ${user} !** Tu passes en **${wonRole.name}** ! Profite bien de ta visibilité pendant 24h ! ✈️✨`);
                
                // Programmer la suppression après 24h
                setTimeout(async () => {
                    try {
                        const m = await channel.guild.members.fetch(user.id).catch(() => null);
                        if (m) {
                            await m.roles.remove(wonRole.id).catch(() => {});
                            console.log(`[Jackpot] Rôle ${wonRole.name} retiré de ${user.tag} après 24h`);
                        }
                    } catch (err) {
                        console.error(`[Jackpot] Erreur lors du retrait du rôle après 24h:`, err);
                    }
                }, 24 * 60 * 60 * 1000);
